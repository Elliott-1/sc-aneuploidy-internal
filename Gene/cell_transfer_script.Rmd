...
```{r}
seurat_cao_dis <- CreateSeuratObject(dis_cao_mtx, meta.data = dis_cao_obs)
```
...
```{r}
dis_cao_cds_final <- preprocess_cds(dis_cao_cds_final, num_dim = 100)
plot_pc_variance_explained(dis_cao_cds_final)
dis_cao_cds_final <- reduce_dimension(dis_cao_cds_final)
```

```{r}
plot_cells(dis_cao_cds_final, genes=c('XIST')) + facet_wrap(~annot, nrow=3)
```

```{r}
saveRDS(dis_cao_cds_final, "D:/newgenes/data/Cao_data/final_cds.RDS")
```
